...
async function createDiffTXT(differences, outputFilePath) {
    ...
    if (onlyInPDF1.length > 0) {
        ...
        textContent.push(onlyInPDF1.map(line => `- ${line}`).join('\n'));
    } else {
    ...
    if (onlyInPDF2.length > 0) {
        ...
        textContent.push(onlyInPDF2.map(line => `- ${line}`).join('\n'));
    } else {
...
async function main() {
    ...
    const outputPath = 'C:/Users/JoséPedroAlvesdaSilv/Desktop/ProjetosWeb/comparerPDF/resultado/output.txt';
